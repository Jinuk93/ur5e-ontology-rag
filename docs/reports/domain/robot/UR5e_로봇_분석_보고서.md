...
| 조인트 위치 | ❌ 미구현 | q0~q5 (rad) |
| 조인트 속도 | ❌ 미구현 | dq0~dq5 (rad/s) |
| 조인트 전류 | ❌ 미구현 | I0~I5 (A) |
| 안전 상태 | ❌ 미구현 | normal, protective_stop |

| Axia80 데이터 항목 | 상태 | 설명 |
|-------------------|------|------|
| **센서값 (6축)** | ✅ 구현됨 | Fx, Fy, Fz, Tx, Ty, Tz |

**중요**: 실제 UR5e 장비는 RTDE(30004), Primary/Secondary/Realtime Interface(30001/30002/30003), Dashboard Server(29999) 등을 통해 조인트 각도/속도/토크, TCP 포즈/속도, 로봇/안전 모드, I/O 상태 등의 **실시간 운영 데이터를 제공**합니다. 다만 **현 시스템은 이 인터페이스와 연결하지 않아** 로봇 텔레메트리를 수집하지 않고, UR5e 에러코드는 온톨로지 규칙 기반 “예측”만 합니다.

**현재 이기종 결합 상태**:
```
[Axia80 센서값]  +  [UR5e 에러코드]  =  이기종 결합 ✅
      ✅                  ✅
  Fx,Fy,Fz            C153, C189
  Tx,Ty,Tz              ...
...
> 본 문서에서 설명하는 UR5e 실시간 운영 데이터(조인트 상태, 속도, 전류 등)는 **향후 확장 시 수집 가능한 데이터**입니다.
