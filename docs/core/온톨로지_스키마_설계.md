...
```mermaid
graph TB
    subgraph Equipment["🔧 Equipment Domain"]
        UR5e[UR5e Robot]
        JOINT[Joints 0-5]
        CB[Control Box]
        TF[Tool Flange]
    ...
    subgraph Measurement["📊 Measurement Domain"]
        AXIA[Axia80 Sensor]
        AXIS[Fx, Fy, Fz, Tx, Ty, Tz]
        STATE[States: NORMAL/WARNING/CRITICAL]
        PAT[Patterns: COLLISION/OVERLOAD/DRIFT]
        AXIA --> AXIS
        AXIS --> STATE
        STATE --> PAT
    end
    ...
    subgraph Knowledge["📚 Knowledge Domain"]
        ERR[99 Error Codes<br/>C001-C099]
        CAUSE[Causes]
        RES[Resolutions]
        ERR --> CAUSE
        CAUSE --> RES
    ...
    subgraph Context["⏰ Context Domain"]
        SHIFT[Shifts A/B/C]
        PROD[Products]
        TIME[Time Patterns]
    end
    ...
    PAT -->|TRIGGERS| ERR
    ERR -->|CAUSED_BY| CAUSE
    CAUSE -->|MITIGATED_BY| RES
    Context -.->|provides context| Equipment
    Context -.->|provides context| Measurement
    Context -.->|provides context| Knowledge
```
