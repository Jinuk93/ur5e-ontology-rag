...
flowchart TB
    subgraph DataSources["📊 데이터 소스"]
        DOC[문서 3종<br/>User/Service/Error Manual]
        SENSOR[Axia80 센서<br/>Fx,Fy,Fz,Tx,Ty,Tz]
        ROBOT[UR5e 로봇<br/>Joint/TCP 데이터]
    end

    subgraph Ontology["🔗 온톨로지 레이어"]
        SCHEMA[4-Domain Schema<br/>Equipment/Measurement<br/>Knowledge/Context]
        ENTITY[99 에러코드<br/>+ 패턴/원인/해결책]
        RULE[추론 규칙<br/>400+ 패턴 매칭]
    end

    subgraph RAG["🔍 RAG 파이프라인"]
        QC[QueryClassifier]
        HR[HybridRetriever<br/>Bi + Cross Encoder]
        OE[OntologyEngine<br/>GraphTraverser + RuleEngine]
        RG[ResponseGenerator<br/>GPT-4o-mini]
    end
...
│                                                                          │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                  │
│  │ Axia80 센서  │  │ UR5e 에러코드│  │  문서 (PDF)  │                  │
│  │ (7일 데이터) │  │ (이벤트포함) │  │ (722 chunks) │                  │
│  │     ✅       │  │     ✅       │  │     ✅       │                  │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘                  │
│         │                 │                 │                           │
│         ▼                 ▼                 ▼                           │
...
> **⚠️ 현재 데이터 상태 (2026-01-27 기준)**
>
> | 데이터 소스 | 상태 | 설명 |
> |------------|------|------|
> | **Axia80 센서값** | ✅ 구현됨 | 7일 시뮬레이션 데이터 (Fx, Fy, Fz, Tx, Ty, Tz) |
> | **UR5e 에러코드** | ✅ 구현됨 | 이벤트 데이터에 포함 (C153, C189 등) |
> | **UR5e 운영 데이터** | ❌ 미구현 | 조인트 위치/속도/전류, 로봇 상태 등 |
> | **문서 (PDF)** | ✅ 구현됨 | 722 chunks 벡터화 완료 |
>
> **현재 "이기종 결합"의 구현 상태**:
> - ✅ Axia80 센서 데이터 (Fx~Tz) + UR5e 에러코드 (C153 등) = **이기종 결합 완료**
> - ❌ UR5e 실시간 운영 데이터 (조인트 상태, 속도, 전류) = **미구현**
...
| **OCCURS_DURING** | Event/Pattern → Shift | 발생 시간 |
| **INVOLVES** | Event/Pattern → Product | 관련 제품 |
| **INSTANCE_OF** | Event → Pattern | 이벤트-패턴 연결 (추적용) |
...
> | 항목 | 상태 | 설명 |
> |------|------|------|
> | **에러 코드** | ✅ 구현됨 | C153, C189 등 (이벤트 데이터에 포함) |
> | 로봇 상태 | ❌ 미구현 | running, idle, stopped, error |
> | 조인트 위치 | ❌ 미구현 | 6개 관절 각도 (q0~q5, rad) |
> | 조인트 속도 | ❌ 미구현 | 6개 관절 속도 (dq0~dq5, rad/s) |
> | 조인트 전류 | ❌ 미구현 | 6개 관절 전류 (I0~I5, A) |
> | 안전 상태 | ❌ 미구현 | normal, protective_stop |
>
> **이기종 결합 현황**: Axia80 센서값 + UR5e 에러코드 = ✅ 결합됨
...
| **Reranker** | Cross-Encoder (BGE-reranker-base) | 2단계 검색, 한국어 지원 |
...
## 9.3 2단계 검색 파이프라인 (Bi-encoder + Cross-encoder)
...
│                    ▼                                                     │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │  Stage 1: Bi-encoder (OpenAI text-embedding-3-small)             │  │
│  │  ────────────────────────────────────────────────────────────────│  │
│  │  • 쿼리/문서 독립 임베딩                                         │  │
│  │  • 빠른 후보 검색 (Top-K=20)                                     │  │
...
│                    ▼ (20개 후보)                                         │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │  Stage 2: Cross-encoder (BGE-reranker-base)                      │  │
│  │  ────────────────────────────────────────────────────────────────│  │
│  │  • 쿼리-문서 쌍 동시 분석                                        │  │
│  │  • 의미적 상호작용 학습                                          │  │
│  │  • 정밀 리랭킹 (Top-N=5)                                         │  │
│  │  • 한국어 지원 (BAAI/bge-reranker-base)                          │  │
│  └───────────────────────────────────────────────────────────────────┘  │
│                    │                                                     │
│                    ▼ (5개 최종 결과)                                     │
│  ┌───────────────────────────────────────────────────────────────────┐  │
...
openai>=1.0.0
langchain>=0.1.0
sentence-transformers>=2.2.0  # Cross-encoder 리랭커
...
| `core/온톨로지_스키마_설계.md` | 온톨로지 스키마 상세 설계 |
| `ontology-chatbot/온톨로지_확장_v2.0.md` | 온톨로지 v2.0 확장 (99개 에러코드) |
| `ontology-chatbot/챗봇_파이프라인_아키텍처.md` | 챗봇 질의응답 파이프라인 상세 |
| `core/Unified_ROADMAP.md` | 개발 로드맵 |
| `steps/step_18_시스템_종합점검.md` | 시스템 종합점검 보고서 |
| `steps/step_19_종합테스트_보고서.md` | 150개 테스트 100% 통과 보고서 |
