...
> **참고 문서**: Axia80_센서_분석_보고서.md, UR5e_로봇_분석_보고서.md
...
| **OCCURS_DURING** | 컨텍스트 | EVT-003 → SHIFT_B |
| **INVOLVES** | 관련 제품 | EVT-003 → PART-C |
...
Endpoints:
  POST /api/query:
    ...
    output: { answer, relations, sources }

  GET /api/ontology/entity/{id}:
    output: { entity, relationships }

  GET /api/sensor/current:
    output: { values, state, patterns }
```
