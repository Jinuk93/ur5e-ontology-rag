...
대한민국 제조업의 미래는 **AX(AI Transformation)**에 있습니다.
기존 룰베이스(IF-THEN) 시스템은 단순 임계값 판단만 가능하지만, 현장에서 필요한 것은:

- "왜 이런 상황이 발생했는가?" (원인 추론)
- "내일 어떤 문제가 생길까?" (예측)
- "지금 이 맥락에서 최선의 조치는?" (맥락 기반 판단)
...
| **2차** | 팔란티어 스타일 관계 시각화 데모 |
| **3차** | 룰베이스 → 온톨로지 전환의 가치 입증 |
...
```
충돌(Collision): Fz가 갑자기 -350N 이하로 급락 (100ms 이내)
과부하(Overload): |Fz|가 150N 초과 상태로 5초 이상 지속
드리프트(Drift): baseline 대비 10% 이상 편차가 30분 이상 유지
진동(Vibration): 표준편차가 평소의 2배 이상으로 10초 이상 지속
```
