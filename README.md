...
대한민국 제조업의 미래는 **AX(AI Transformation)** 에 있습니다. 기존 룰베이스(IF-THEN) 시스템은 단순 임계값 판단만 가능하지만, 실제 현장에서 필요한 것은:
...
│                                                                          │
│  1️⃣  이기종 데이터 통합                                                   │
│      └─ UR5e 로봇 + Axia80 센서 → 온톨로지로 관계 정의                    │
│                                                                          │
│  2️⃣  관계 기반 추론                                                       │
│      └─ "Fz=-350N" → 충돌 패턴 → C153 에러 가능성 → 권장 조치             │
│                                                                          │
│  3️⃣  근거 있는 예측                                                       │
│      └─ 모든 응답에 온톨로지 경로 + 문서 출처 제시                         │
│                                                                          │
│  4️⃣  PoC로 가치 입증                                                      │
│      └─ 룰베이스 → 온톨로지 전환의 실질적 이점 검증                        │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                          │
│  단일 장비 분석의 한계:                                                   │
│  ├─ Axia80만: "힘이 급증했다" → 왜? 알 수 없음                           │
│  └─ UR5e만: "속도가 줄었다" → 원인? 알 수 없음                           │
│                                                                          │
│  이기종 결합 분석:                                                        │
│  └─ "고속 이동(tcp_speed↑) 중 접촉(Fz↓) → 충돌 패턴"                     │
│      → 패턴 기반 예측 가능                                               │
│      → 예방 보전 실현                                                    │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
